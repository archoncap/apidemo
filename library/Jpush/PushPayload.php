...
class Jpush_PushPayload {
    ...
    public function setPlatform($platform) {
        if (is_string($platform) && strcasecmp("all", $platform) === 0) {
            ...
            if (!is_array($platform)) {
                ...
                if (count($platform) <= 0) {
                    throw new InvalidArgumentException("Missing argument for Jpush_PushPayload::setPlatform()");
                }
            }
    ...
    public function send() {
        $response = $this->client->_request(Jpush_PushPayload::PUSH_URL, JPush::HTTP_POST, $this->toJSON());
        return $this->__processResp($response);
    }

    public function validate() {
        $response = $this->client->_request(Jpush_PushPayload::PUSH_VALIDATE_URL, JPush::HTTP_POST, $this->toJSON());
        return $this->__processResp($response);
    }
    ...
    private function __processResp($response) {
        if($response['http_code'] === 200) {
            ...
            throw new Jpush_APIRequestException($response);
        }
    }
