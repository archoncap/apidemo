...
class ImageManager {
	...
	public static function thumbnail($image, $cache_image, $size, $image_type = 'jpg') {
		define('IMAGE_CACHE_DIR', '.');
		$image 		= (strpos($image,'./')===FALSE) ? IMAGE_CACHE_DIR . $image : $image;
		$cache_image= (strpos($cache_image,'./')===FALSE) ? IMAGE_CACHE_DIR . $cache_image : $cache_image;
		...
		if (!file_exists($cache_image))
		{
			...
			else
			{
				...
				ImageManager::resize($image, $cache_image, $ratio_x, $size, $image_type);
			}
		}

		return str_replace('./', '/', $cache_image);
	}
