...
abstract class Db {
	...
	public static function getInstance() {
		...
		if (!isset(self::$instance))
		{
			...
			self::$instance = new $class(self::$_servers['server'], self::$_servers['user'], self::$_servers['password'], self::$_servers['database']);
		}
	...
	public function __construct($server, $user, $password, $database, $connect = true) {
		...
		$this->is_cache_enabled = OBJECT_CACHE_ENABLE;
		$this->log_error = Yaf_Registry::get('database')['mysql_log_error'];
	 ...
	 */
	public function getAll($sql, $use_cache = true) {
		...
		$this->last_query = $sql;		
		$md5sqlkey	=	md5($sql);
		if ($use_cache && $this->is_cache_enabled && Cache::getInstance()->exists($md5sqlkey))
		{		
			$this->last_cached = true;			
			return Cache::getInstance()->get($md5sqlkey);
		}
	 ...
	 * @return array
	 */
	public function getFields($table_name, $use_cache = true) {
	 ...
	 * @return bool|mixed
	 */
	public function getPrimaryKey($table_name, $use_cache = true) {
		...
		if ($use_cache && $this->is_cache_enabled)
			Cache::getInstance()->setQuery($sql, $Key);
